...
int main(int argc, char **argv) {
    ...
    // Allocate unified memory for the row pointer of sparse matrix A
    gpuCheck(cudaMallocManaged(&ARowPtr, (dimX + 1) * sizeof(int)));
    // Allocate unified memory for column indexes of sparse matrix A
    ...
    // Declare the cuBLAS/cuSPARSE handles and descriptors
    cublasHandle_t cublasHandle;      // cuBLAS handle
    cusparseHandle_t cusparseHandle;  // cuSPARSE handle
    cusparseSpMatDescr_t Adescriptor; // Mat descriptor
    cusparseDnVecDescr_t Tdescriptor; // Vec descriptor
    cusparseDnVecDescr_t Ydescriptor; // Imm Vec descriptor
